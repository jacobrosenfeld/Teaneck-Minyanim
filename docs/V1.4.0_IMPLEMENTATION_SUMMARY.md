...
**Organization Calendar View** (`/admin/{orgId}/calendar-events`):
- ✅ Proper page header with display-4 styling
- ✅ Lead paragraph describing functionality
- ✅ Access to settings (siteName, supportEmail) via @ModelAttribute
- ✅ View all materialized events with comprehensive filtering
- ✅ Filter by date range, minyan type, source, enabled status
- ✅ Statistics dashboard showing event counts by source
- ✅ Enable/disable toggle for any event
- ✅ Inline editing of notes and location
- ✅ Delete functionality for manual events only
- ✅ Manual rematerialization trigger button
- ✅ Visual indicators for manually edited events
- ✅ Color-coded badges for source and type
- ✅ Added to admin sidebar navigation

**Master Calendar View** (`/admin/calendar-events/all`):
- ✅ Proper page header with display-4 styling
- ✅ Lead paragraph with siteName integration
- ✅ Access to settings via @ModelAttribute
- ✅ View events across all organizations
- ✅ Organization filter dropdown
- ✅ Organization name in table
- ✅ Same management capabilities as org view
- ✅ Enable/disable events for any organization
- ✅ Added to admin sidebar for super admins
...
**Key Changes Needed:**
1. Update `ZmanimService.getZmanim()` to query from `calendar_events` table
2. Update `ZmanimService.org()` to query from `calendar_events` table
3. Add `shouldDisplayEvent()` helper for time-window filtering
4. Remove dual code path logic (calendar import vs rules)
5. Add out-of-window date handling with friendly messages

### Testing (Future)
- Unit tests for CalendarMaterializationService
- Integration tests for precedence logic
- Tests for delete+rebuild strategy
- Tests for rolling window boundaries
