...
### Fixed
- **Deleted source calendar events (#240)**: Scheduled calendar imports now reconcile the successful feed coverage window, mark imported rows missing from the source calendar as source-deleted/disabled, and disable stale imported `calendar_events` rows so removed minyanim no longer remain live.
