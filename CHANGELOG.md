...
### Added
- **Admin passwordless auth (#243)**: Added passkey signin via Spring Security WebAuthn, passkey registration through `/webauthn/register`, magic-link signin wired into the existing email handler, normalized account email lookup, auth migration metadata, login timestamp/method fields, identifier-first login method detection, and migration SQL while keeping `ACCOUNT`/`TNMUser` as the source of truth for the future user-management page planned in #246.
...
- **Admin login passwordless fallback (#243)**: Reworked the admin login page so Edge and other WebAuthn-capable browsers are not blocked by the Spring helper script guard, passkey cancellation falls through to magic-link delivery, and password login accepts the same email-or-username identifier.
