...
public class ZmanimService {
    ...
    public ModelAndView getZmanim(Date date) {
        ...
        for (Organization org : allOrganizations) {
            ...
            // Query materialized calendar events (same source as main minyan list)
            List<com.tbdev.teaneckminyanim.model.CalendarEvent> calendarEvents = 
                effectiveScheduleService.getEffectiveEventsForDate(orgId, localDateRef);
            ...
            for (MinyanEvent event : orgEvents) {
                if (event.getStartTime() != null) {
                    ...
                    kolhaMinyanims.add(new KolhaMinyanim(
                        "cal-event", // Simple identifier for materialized events
                        event.getType(),
    ...
    public ModelAndView org(String orgId, Date date) throws Exception {
        ...
        // Query materialized calendar for this organization
        log.info("Querying materialized calendar for organization: {}", orgId);
        LocalDate localDateRef = dateToLocalDate(date);
        List<com.tbdev.teaneckminyanim.model.CalendarEvent> calendarEvents = 
            effectiveScheduleService.getEffectiveEventsForDate(orgId, localDateRef);
    ...
    /**
     * Helper method to determine if an event should be displayed based on time windows.
     * This preserves existing filtering logic for Shacharis/Mincha/Maariv/Selichos.
     *
     * @param event The MinyanEvent to check
     * @param zmanim Map of Zman times for the date
     * @param isSelichosRecited Whether Selichos is being recited
     * @return true if the event should be displayed
     */
    private boolean shouldDisplayEvent(MinyanEvent event, Dictionary<Zman, Date> zmanim, boolean isSelichosRecited) {
        if (event.getStartTime() == null) {
            return false;
        }
        ...
        String dynamicTime = event.dynamicTimeString();
        
        // Calculate time boundaries
        Calendar shekiyaMinusOne = Calendar.getInstance();
        shekiyaMinusOne.setTime(zmanim.get(Zman.SHEKIYA));
        shekiyaMinusOne.add(Calendar.MINUTE, -1);
        
        Calendar mgMinusOne = Calendar.getInstance();
        ...
        if (type.equals(MinyanType.SELICHOS)) {
            return isSelichosRecited;
        }
